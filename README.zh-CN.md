...
Swift + AppKit 原生 macOS 多视频剪辑与中英法字幕编辑器。深色三栏工作区，AVPlayer 预览，双语字幕时间轴，原生 H.264/AAC 与 HEVC 10-bit 烧录导出。
...
2. 点击 **生成字幕**，在弹窗中选择视频语言（**法语 / 英语**）和目标语言（**中文 / 英语 / 法语**），新工程默认 **法语 → 中文**。源目标相同时只转写，不重复生成字幕。应用本地 Whisper 转写后，通过已登录的 Codex 和新多语言 skill 翻译；可取消和重试，切换语言会新建独立任务，避免混用旧缓存。语言组合随工程保存，时间轴和预览开关按所选语言显示。
...
6. 预览右上的**字幕语言开关** 保持独立开关：只选法语显示法语，只选中文显示中文，两者都选显示双语，两者都关闭则隐藏字幕。时间轴、预览和导出同步遵循选择；左侧分类切换不改变这些开关。播放或拖动进度时，时间轴与属性跟随当前字幕；播放时预览不显示选中框。开始编辑文本或时间数值时自动暂停播放。宽度、位置、字体、字号、颜色、描边和背景的修改自动应用于同语言所有字幕，并清除该语言已有的单条样式覆盖；另一种语言不受影响。文字内容与起止时间仍只修改当前字幕。
...
- Codex：优先使用有效的已配置路径；失效后自动查找 `/Applications`、`~/Applications` 中 ChatGPT/Codex 应用的新旧目录、PATH 及常见 CLI 安装位置。设置显示检测结果，未找到时仍提示手动配置。需预先登录；沿用默认模型与登录，不需 API Key。
...
工程恢复、任务日志、临时音频及内部 SRT 存放在 `~/Library/Application Support/VideoEditeur/`。每次启动进入空白新工程，不自动恢复上次工程或生成任务。可通过「打开工程」（⇧⌘O）手动打开已保存的 `.frzh` 或上述目录中的 `Recovery.frzh`；自动保存和未完成任务的批次缓存仍保留。人工编辑后会开始新任务，以免重试覆盖新修改。切换视频前未命名工程会归档到 `Recovered Projects/`。任务缓存不会自动清理，可在不运行任务时自行清理 `Jobs/`。
...
时间轴顶部「新建文字轨道」会添加独立轨道和一段两秒说明文字，默认使用 Arial 字体；已有轨道保留原字体。选中片段后在右侧编辑内容、起止时间、字体和位置；拖动片段移动时段，拖动左右边缘调整显示时长。用轨道下拉框选择目标，再点「添加文字」可在播放位置继续添加。同轨片段不能重叠，不同轨道可以同时显示。文字轨道随工程保存，并烧录到导出视频；法语／中文开关和重新生成字幕不影响文字轨道。可用左侧删除按钮删除选中文字片段，撤销可恢复。
...
默认中文，不随系统语言自动改变。通过应用菜单「字幕工坊 → 界面语言 → 中文 / English」，或齿轮设置中的「界面语言」选择；保存后重启应用生效，并记住选择。英文模式覆盖菜单、主界面、属性、剪辑、去字、生成进度和应用错误提示；系统文件选择器等由 macOS 控制。界面语言独立于字幕语言，不翻译已有字幕、工程名称或文件路径，不改变生成字幕时选择的语言。
...
新工程默认隐藏 FR/ZH 时间轴行，点击「生成字幕」后展开；打开已有字幕的工程时自动显示。

关闭窗口或退出 App 时，若工程有未保存的修改，会提示「保存 / 不保存 / 取消」；取消保存对话框也会取消退出。自动保存仅写入恢复副本，不覆盖正式工程文件；点击「保存」才更新正式文件。空白或未修改的已保存工程直接退出。

新 skill 源码位于 `Assets/Skills/video-generate-multilingual-subtitles/`，随应用打包。原默认 skill 路径自动使用内置新版；自定义转写脚本路径继续保留，翻译使用内置多语言规则。英语字幕默认 Arial、字号 70、宽度 98%。

可从 Finder 直接把视频、音乐拖到时间轴内容区域导入，与拖入素材列表一致。支持多文件混合拖入：视频追加主视频轨道末尾，音乐从当前播放头开始并保留完整源时长。可接收时高亮时间轴；生成/导出期间禁止导入，主视频轨道锁定时仍允许音乐导入。素材列表同步显示，可撤销。
